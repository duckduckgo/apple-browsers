...
final class NativeDuckPlayerNavigationHandlerTests: XCTestCase {
    ...
    func testHandleURLChange_WhenFeatureOff_ReturnsNotHandled() {
        ...
        let url = URL(string: "https://www.youtube.com/watch?v=123")!
    ...
    func testHandleURLChange_WhenSERPDisabled_ReturnsNotHandled() {
        // Given
        mockFeatureFlagger.enabledFeatures = [.duckPlayer]
    ...
    func testHandleDuckNavigation_LoadsYouTubeURL() {
        ...
        // Then
        XCTAssertEqual(mockWebView.lastLoadedRequest?.url?.absoluteString, "https://www.youtube.com/watch?v=\(videoID)")
    }

    // MARK: - handleDuckNavigation Additional Tests

    func testHandleDuckNavigation_WithTimestamp_LoadsYouTubeURLWithTimestamp() {
        // Given
        let videoID = "123"
        let timestamp = "60"
        let url = URL(string: "duck://player/\(videoID)")!
    ...
    func testHandleDelegateNavigation_WhenSERPEnabled_HandlesYouTubeURL() {
        // Given
        mockFeatureFlagger.enabledFeatures = [.duckPlayer]
    ...
    // MARK: - handleDelegateNavigation Additional Tests

    func testHandleDelegateNavigation_NonMainFrame_DoesNotHandle() {
        // Given
        mockFeatureFlagger.enabledFeatures = [.duckPlayer]
        let url = URL(string: "https://www.youtube.com/watch?v=123")!
    ...
    func testHandleAttach_InitializesCorrectly() {
        ...
        // Then
        // Verify that the handler is properly initialized with the web view
        XCTAssertNotNil(sut)
        // Add more specific assertions based on what handleAttach should do
    }
    ...
    func testDuckURLNavigation_RedirectsToMatchingYouTubeVideo() {
        // Given
        let videoID = "abc123"
        let duckURL = URL(string: "duck://player/\(videoID)")!
        let expectedYouTubeURL = "https://www.youtube.com/watch?v=\(videoID)"
        ...
        // Then
        XCTAssertEqual(mockWebView.lastLoadedRequest?.url?.absoluteString, expectedYouTubeURL)
        XCTAssertEqual(mockWebView.loadCallCount, 1)
    }

    func testDuckURLNavigation_WithTimestamp_RedirectsToMatchingYouTubeVideoWithTimestamp() {
        // Given
        let videoID = "abc123"
        let timestamp = "45"
        let duckURL = URL(string: "duck://player/\(videoID)?t=\(timestamp)")!
        let expectedYouTubeURL = "https://m.youtube.com/watch?v=\(videoID)&t=\(timestamp)"
        let navigationAction = MockNavigationAction(request: URLRequest(url: duckURL))
