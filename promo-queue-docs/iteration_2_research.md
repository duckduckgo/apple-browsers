...
> **Iteration-one update — 2026-08-12:** The original research correctly found that RMF itself has no cooldown/frequency engine. Q2 now uses a startup-latched mode, one singular global modal/RMF owner, and one service-owned logical RMF session that authorizes exactly one renderer. Q3 implements the fixed client-side matrix (modal→RMF 10m, RMF→RMF 10m, RMF→modal 24h, existing modal→modal interval) outside RMF from confirmed history, with checkpoint-only reconsideration. A same-message renderer handoff remains one logical session; host exposure still determines renderer eligibility. Promo Queue telemetry remains separate. This does not change the broader RMF gap findings below.
...
1. **Iteration 1 (two active stacked PRs; DRI Bartosz):** "stop the bleed" — extend the existing iOS launch-modal coordination with a small app-level owner so NTP RMF and launch modals cannot overlap, then add the fixed directional matrix in the final PR. `Foreground`/`UIInteractionManager` retain lifecycle readiness; `PromoCoordinationService` owns one logical RMF session and renderer selection plus the narrow cross-surface policy; `ModalPromptCoordinationManager` keeps provider eligibility, priority, existing modal cooldown, and exact-root presentation lifetime. NTP models report candidates and host eligibility and render only a service-authorized presentation. PR 1 is merged; `bartosz/promo-q-2` establishes the central logical owner and OS-specific exact removal; stacked `bartosz/promo-q-3` adds cooldowns and diagnostics without telemetry. macOS `PromoService` is **not** extracted for iteration 1. Design: `promo-queue-docs/TECH_DESIGN_FINAL.md`; final-slice record: `promo-queue-docs/Q3_IMPLEMENTATION_PLAN.md`.
