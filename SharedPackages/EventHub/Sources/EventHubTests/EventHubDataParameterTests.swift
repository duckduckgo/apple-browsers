...
struct EventHubDataParameterTests {
    ...
    func immediateDataParamEncodesStringValue() {
        ...
        f.manager.handleWebEvent(EventHubManagerFixture.eventWithData("login", dataJSON: #"{ "loginState": "logged-in" }"#), tabID: .new())
    ...
    func immediateDataParamEncodesObjectValue() {
        ...
        f.manager.handleWebEvent(EventHubManagerFixture.eventWithData("login", dataJSON: #"{ "payload": { "a": true } }"#), tabID: .new())
        #expect(f.fired.count == 1)
    ...
    func immediateDataParamEncodesNullValue() {
        let f = EventHubManagerFixture.active(Self.immediateDataConfig)
        f.manager.handleWebEvent(EventHubManagerFixture.eventWithData("login", dataJSON: #"{ "loginState": null }"#), tabID: .new())
        #expect(f.fired.count == 1)
    ...
    func immediateDataParamOmittedWhenKeyAbsent() {
        let f = EventHubManagerFixture.active(Self.immediateDataConfig)
        f.manager.handleWebEvent(EventHubManagerFixture.eventWithData("login", dataJSON: #"{ "other": "x" }"#), tabID: .new())
        #expect(f.fired.count == 1)
    ...
    func aggregateDataParamUsesLastValueFromMatchingSource() {
        ...
        """
        let f = EventHubManagerFixture.active(config)
        f.manager.handleWebEvent(EventHubManagerFixture.eventWithData("yt", dataJSON: #"{ "loginState": "a" }"#), tabID: .new())
        f.manager.handleWebEvent(EventHubManagerFixture.eventWithData("yt", dataJSON: #"{ "loginState": "b" }"#), tabID: .new())
        f.advance(by: 60)
