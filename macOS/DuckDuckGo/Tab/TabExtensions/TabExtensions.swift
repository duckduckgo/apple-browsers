...
extension TabExtensionsBuilder {
    ...
    mutating func registerExtensions(with args: TabExtensionsBuilderArguments, dependencies: TabExtensionDependencies) {
        ...
        if let hotspotDetectionService = dependencies.hotspotDetectionService {
            add {
                let captivePortalHandler: CaptivePortalHandler
                if let popupManager = dependencies.captivePortalPopupManager {
                    captivePortalHandler = DefaultCaptivePortalHandler(popupManager: popupManager)
                } else {
                    // Fallback for tests or edge cases where no shared manager is available
                    captivePortalHandler = DefaultCaptivePortalHandler(popupManager: CaptivePortalPopupManager())
                }
