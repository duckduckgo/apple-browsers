...
public class SitePerformanceTester: NSObject {
    ...
    private func collectPerformanceMetrics() async -> DetailedPerformanceMetrics? {
        guard let url = Bundle(for: SitePerformanceTester.self).url(forResource: "detailedPerformanceMetrics", withExtension: "js") else {
            logger.error("Failed to find detailedPerformanceMetrics.js in bundle")
            return nil
        }

        guard let script = try? String(contentsOf: url) else {
            logger.error("Failed to read detailedPerformanceMetrics.js from bundle")
            return nil
        }
