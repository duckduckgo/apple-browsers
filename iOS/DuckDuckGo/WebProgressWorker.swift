...
class WebProgressWorker {
    ...
    weak var progressBar: ProgressView? {
        didSet(oldValue) {
            ...
            if isLoading {
                // Cap initial progress below 1.0 so finishAndHide can always animate to completion
                let cappedProgress = min(currentProgress, 0.9)
                self.progressBar?.show(initialProgress: cappedProgress)
            } else {
