...
extension TabCollectionViewModel {
    ...
    var hasTabsWithNonInternalTabContentOrNonEmptyBackForwardHistory: Bool {
        func isNonInternalTabContent(_ content: Tab.Content) -> Bool {
            switch content {
        ...
        return tabCollection.tabs.contains {
            return isNonInternalTabContent($0.content) || $0.backHistoryItems.contains(where: { isNonInternalTabContent(.contentFromURL($0.url, source: .historyEntry)) })
            || $0.forwardHistoryItems.contains(where: { isNonInternalTabContent(.contentFromURL($0.url, source: .historyEntry)) })
        }
    }
}
