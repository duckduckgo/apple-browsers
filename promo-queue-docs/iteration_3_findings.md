...
> **Iteration-one status update — 2026-08-12:** The finding that RMF itself lacks cooldown/frequency policy remains true. Q2 is startup-latched and uses one singular transient owner plus one service-owned logical RMF session that authorizes exactly one renderer. Q3 implements the fixed launch-modal/NTP-RMF matrix from confirmed history with checkpoint-only reconsideration. Same-message renderer transfer keeps one session and one queue timestamp; host exposure remains the eligibility input. Telemetry remains separate. This narrow precedent does not satisfy the broader B/B+/C requirements evaluated below.
