...
final class SubscriptionPagesUseSubscriptionFeatureV2: Subfeature {
    ...
    func featureSelected(params: Any, original: WKScriptMessage) async throws -> Encodable? {
        ...
        case .paidAIChat:
            PixelKit.fire(SubscriptionPixel.privacyProWelcomeAIChat, frequency: .uniqueByName)
    ...
    func showSubscriptionFoundAlert(originalMessage: WKScriptMessage) async {
        ...
        case .alertFirstButtonReturn:
            if #available(macOS 12.0, *) {
                ...
                switch result {
                case .success:
                    PixelKit.fire(SubscriptionPixel.privacyProRestorePurchaseStoreSuccess, frequency: .legacyDailyAndCount)
                case .failure(let error):
