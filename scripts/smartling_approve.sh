...
output=$(./scripts/loc_tool.sh approve --job-id "$JOB_ID" 2>&1) || approve_failed=1

echo "$output"

if [ "${approve_failed:-0}" = "0" ] && echo "$output" | grep -q "APPROVED=1"; then
    # Generate success message
    ./scripts/smartling_messages.sh approve approve_message.txt "$PLATFORM" "$JOB_ID" "$SMARTLING_PROJECT_ID" success
    echo "✅ Job approved successfully"
    exit 0
else
    # Generate error message
    ./scripts/smartling_messages.sh approve approve_message.txt "$PLATFORM" "$JOB_ID" "$SMARTLING_PROJECT_ID" failed
