...
///
/// The script runs in two places: on the extension controller's web view configuration, which
/// covers the pages WebKit creates for the extension (background page, action popup, options page
/// and iframes inside them), and as a tab user script, which covers an extension page loaded as an
/// iframe inside a website — iCloud Passwords' completion list, for instance. In a tab it returns
/// right away unless the frame is an extension page (`webkit-extension:`), so websites are
/// untouched. In an embedded frame it also hides `action.openPopup`; see the note at that block.
public enum WebExtensionAPIStubScript {
    ...
    public static let retentionPropertyName = "__ddgRetainedExtensionAPINamespaces"
    ...
    (function() {
        ...
        // they must not get stubs installed on it, nor the retention array leaked into them. Only
        // extension pages are served over `webkit-extension:`. A bare `JSContext` in tests has no
        ...
        // iframe loses it too, at no cost. The controller's web view configuration does not cover
        // a frame inside a tab, so this block only reaches the completion list through the tab
        // user script the app installs (`WebExtensionPageStubUserScript`).
        var hiddenMembers = [];
        try {
