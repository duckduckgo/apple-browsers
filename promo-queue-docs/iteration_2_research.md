...
> **Iteration-one update — 2026-08-12:** The original research correctly found that RMF itself has no cooldown/frequency engine. The accepted Q2 endpoint now uses a startup-latched mode and one singular global modal/RMF owner; its formerly separate fixes PR has been folded into `bartosz/promo-q-2`. The fixed client-side matrix (modal→RMF 10m, RMF→RMF 10m, RMF→modal 24h, existing modal→modal interval) is implemented in stacked `bartosz/promo-q-3`, outside RMF, from confirmed modal/RMF history and reconsidered only at real checkpoints. Promo Queue telemetry remains a separate project. This does not change the broader RMF gap findings below.
...
1. **Iteration 1 (two active stacked PRs; DRI Bartosz):** "stop the bleed" — extend the existing iOS launch-modal coordination with a small app-level cross-surface owner so NTP RMF and launch modals cannot overlap, then add the fixed directional matrix in the final PR. `Foreground`/`UIInteractionManager` retain lifecycle readiness; `PromoCoordinationService` keeps launch-source/unrelated-modal gates and owns the narrow cross-surface policy; `ModalPromptCoordinationManager` keeps provider-level onboarding eligibility, priority, existing modal cooldown, and presentation while holding ownership until exact-root release; NTP reports actual RMF visibility and waits unconsumed while blocked. PR 1 is merged; consolidated `bartosz/promo-q-2` establishes startup-latched singular ownership and truthful physical release; stacked `bartosz/promo-q-3` adds the cooldown policy, animation correction, and diagnostics without telemetry. macOS `PromoService` is **not** extracted for iteration 1. Design: `promo-queue-docs/TECH_DESIGN_FINAL.md`; final-slice plan: `promo-queue-docs/Q3_IMPLEMENTATION_PLAN.md`.
