...
- backgrounding retains a still-valid RMF publication and ownership while disarming fresh admission until another explicit NTP preparation;
...
For ordinary background/foreground within one process, background disables fresh admission but retains a valid source-owned RMF publication, ownership aggregate, lease, acquisition identity, presentation context, and appearance history. There is no stored preparation or host state to clear. Foreground re-enables admission and synchronously revalidates that owner before launch-modal provider evaluation. A still-valid owner is retained without reacquisition or cooldown evaluation. An invalid owner is unpublished and released through ordered teardown, and foreground does not select a replacement without a later normal NTP preparation.
...
- backgrounding preserves a still-valid RMF lease, identity, publication, and appearance history; and
...
| After-idle fallback needs stable trigger semantics. | The source pins only the actual selected filter (`noTrigger` or `afterIdle`) in current ownership; it stores no ownerless preparation policy. | An after-idle request can fall back to a no-trigger candidate. Pinning the actual filter prevents a later after-idle candidate from displacing that still-valid fallback owner, while ownerless retries wait for another explicit preparation. |
...
| Message invalidation releases the card; background only disables fresh admission. | All authoritative endings use unpublish → synchronous source signal → release → clear. Background retains a still-valid owner, publication, identity, and appearance history. Foreground revalidates that retained owner and enables the next explicit preparation. | Ordered teardown prevents a modal from acquiring while stale RMF content is still published. Retention avoids manufacturing a fresh acquisition merely because the app crossed a lifecycle boundary. |
...
| Option B is presented as matching Android. | Android is used only as evidence for claim-before-publication and source-owned over-hold. iOS also retains a valid RMF on background but deliberately differs on message identity, cooldown confirmation, and modal-root lifetime. | Inspection showed Android's app-scoped coordinator is type-keyed and its selection model does not have iOS's multiple independently acting NTP entry points with different preparation policies. Copying all assumptions would be unsafe on iOS. |
...
This is a faithful implementation of the initial simplified direction, not a literal transcription of its sample interfaces. It keeps every defining idea: one app-scoped slot, admission before modal presentation and RMF publication, message/source-lifetime RMF ownership, background retention, no renderer visibility or handoff state, actual-appearance confirmation, weak-token recovery, and acceptance of bounded over-hold. iOS strengthens modal timing by admitting before provider side effects.
...
| Background/view disappearance | Neither releases | Neither releases a still-valid RMF; background disables fresh admission and foreground revalidates retained ownership |
...
Represent reconciliation reason and owned selection with two distinct private concepts:

- the current explicit preparation policy: `noTriggerOnly` or `afterIdleThenNoTrigger`, which exists only for that call; and
...
5. Only for an explicit preparation, fetch a fresh candidate using that call's policy and record the actual filter that selected it. Store-change and foreground reconciliation stop after owner validation or teardown.
...
An enabled explicit `prepareForNTP` is the only operation permitted to transition RMF from ownerless to owned. A disabled call is a no-op and cannot be replayed later. No preparation policy is retained. Therefore a message arriving after an empty preparation, a modal- or cooldown-denied attempt, and a different-ID replacement all wait for the next natural explicit preparation. Same-ID refresh and authoritative invalidation remain immediate.

On app background, the configuration disables fresh RMF admission and retains a still-valid publication, lease, acquisition identity, presentation context, and appearance history. There is no preparation or host state to clear. Store notifications while disabled may refresh or invalidate the retained owner and refresh non-RMF data, but cannot acquire a replacement. Foregrounding re-enables future admission and revalidates a retained owner; it cannot acquire while ownerless. A foreground-visible NTP may subsequently make its ordinary explicit preparation call. This is one app-scoped lifecycle integration through `MainCoordinator`, not a callback from every renderer.
...
The old ID is unpublished from every model snapshot and released during the store notification. Reconciliation then stops. The new ID waits for the next explicit preparation, where it is a fresh acquisition and must pass the RMF incoming cooldown. A same-ID content refresh keeps the current ownership and appearance state while still notifying models when shared content changes.

### Background and foreground

Backgrounding retains a still-valid coordinated RMF in `homeMessages` together with its lease, identity, context, and appearance history. It records no new shown or cooldown history. RMF admission remains disabled while the app is inactive, so ownerless notification processing cannot acquire a card and invalidation cannot acquire a replacement.

Foregrounding re-enables future explicit NTP preparation and revalidates a retained owner, but does not itself acquire or publish while ownerless. If foreground validation invalidates the retained owner, it performs ordered teardown and stops. The existing foreground-visible-NTP checkpoint may then call `prepareForNTP`. A denied preparation does not schedule a timer; an already-mounted NTP can therefore remain without RMF beyond ten minutes until another natural checkpoint. Ordinary NTP disappearance still does not release ownership.
...
The first valid returned-lease `markShown()` call records the RMF timestamp through the service-owned wrapper. RMF acquisition, denial, mapping, dismissal, backgrounding, or lease release does not.
...
- an explicit NTP source preparation, which is the only fresh-acquisition checkpoint;
- a coordinated `remoteMessagesDidChange` refresh, only to refresh or invalidate existing ownership and non-RMF content;
- candidate dismissal, expiry, replacement, or configuration update, which may end ownership but cannot acquire a replacement;
...
- An active RMF may hold the slot while the user is off the NTP or while the app is backgrounded. Only authoritative message invalidation releases that live ownership.
- Background and foreground retain a valid appeared or never-appeared owner. If validation ends ownership, no replacement is acquired until the next explicit preparation. No boundary timer exists, so an ownerless NTP may remain blank beyond a cooldown or arrival until another natural checkpoint.
...
Rejected because per-view release would require tracking which of several physical renderers still represents the shared message. There is no container teardown callback or background-release exception; the source retains a valid owner until authoritative message invalidation.
...
- background retention preserves publication, lease, identity, and appearance history while disarming fresh admission; inactive invalidation releases without replacement;
...
Manual/static validation must exercise the standard NTP plus all three conditional-host families: legacy/iPad `MainViewController` suggestion tray, `OmniBarEditingStateViewController`/`SuggestionTrayManager`, and unified input. Also cover Fire mode, landscape as behavior discovery, appeared and never-appeared background retention, inactive invalidation without replacement, and delayed-retry behavior. Add a cheap host-level automated assertion only where an existing harness already makes it natural.
