...
4. **Classify severity** (use both user count and new-vs-pre-existing):
   ...
   **Procedure per tier — severity does not change which steps run.** The gate that drops an issue from the full pipeline is *step 8's skip rules* (generic culprit, no first-party frames, Jetsam OOM, fix-already-shipped bucket), not the severity tier. In particular:
   - **HIGH and MEDIUM follow the *same* path:** step 5 pre-flight lookup → step 6 git blame → step 8 subagent RCA (when ≥3 first-party frames) → step 9 tracking task → main-report line **leads with the `Tracking` link**. The only difference between HIGH and MEDIUM in the output is the section heading they appear under.
   - **LOW:** no step-5 lookup is wasted on it, no git blame, no subagent, no tracking task. Main-report line leads with the Sentry short-ID.
   - **Pre-existing:** no tracking task, no subagent, **no blame attribution at all** — not full names, not initials, not "assigned to X". List by user count only. Pre-existing issues are owned by their existing tracking tasks; the daily summary just records volume.

   **Low user count is NOT a skip reason for MEDIUM.** Single-occurrence (1 user, 1 event) is the *definition* of MEDIUM, not an exit ramp from the pipeline. A MEDIUM with 1 user gets the same treatment as a MEDIUM with 9 users: tracking task + RCA. Do not invent thresholds like "1-user threshold = no tracking" or "low-volume = monitor only" — those rules do not exist in this skill, and inventing them silently drops the work this skill is meant to produce. The legitimate skips are the four in step 8, full stop.
...
| Skipping tracking-task creation for MEDIUM issues because they're "low-volume" or "single-user" | The MEDIUM tier is *defined* as new-in-version + single occurrence + app-code culprit. Single-user is the entry condition for MEDIUM, not a justification to drop it from the pipeline. MEDIUM follows the **same path as HIGH**: step 5 lookup, step 6 blame, step 8 subagent RCA, step 9 tracking task, main-report line leads with the `Tracking` link. The only legitimate skips are the four in step 8 (generic culprit, no first-party frames, Jetsam OOM, fix-already-shipped bucket). Do **not** fabricate user-count thresholds like "1-user threshold = monitor only" or "no tracking task warranted at this volume" — those rules do not exist in this skill. If MEDIUM clusters exist in a run, tracking tasks must be filed for each, with RCA when step 8 eligibility passes. |
| Attributing blame on pre-existing crashes (full names, initials, or assignee mentions) | Step 4 says pre-existing entries list by user count and do **not** attribute blame at all. No "assigned to Kieran", no "(KS)", no "owned by frontend team". Pre-existing issues already have owners on their own tracking tasks; the daily summary records volume only. Full names additionally get blocked by the asana-exfiltration hook (step 12). The blame columns (initials + PR links) apply to HIGH/MEDIUM new-in-version issues only. |
| Linking Sentry short-IDs without the `APPLE-IOS-` / `APPLE-MACOS-` platform prefix in the URL path | The full short-ID is `APPLE-IOS-DF5F` or `APPLE-MACOS-BE7`, not `DF5F` / `BE7` alone. Use `https://errors.duckduckgo.com/organizations/ddg/issues/APPLE-IOS-DF5F/?project=apple-ios` — the link **text** can shorten to the trailing portion (`DF5F`) for readability, but the URL path must carry the full prefixed short-ID or it may not resolve. Inconsistency within one report (some links prefixed, some not) is a tell that the model dropped the prefix mid-list. |
