...
> **Iteration-one update — 2026-08-06:** The original research correctly found that RMF itself has no cooldown/frequency engine. PR 3 implements a narrow client-side matrix for only launch modals and NTP RMF in `06a2417373`: modal→RMF 10m, RMF→RMF 10m globally, RMF→modal fixed 24h, and modal→modal the existing remote-tunable interval (currently/default 24h). Confirmed modal/RMF source timestamps are persisted by a service-owned policy outside RMF and the transient arbiter. Commit `1f12bf8a66` removes the proposed collision pixels; Promo Queue telemetry is a separate project. This does not change the broader RMF gap findings below.
...
1. **Iteration 1 (PR 3 implementation complete; DRI Bartosz):** "stop the bleed" — extend the existing iOS launch-modal coordination with a small app-level cross-surface permit coordinator so NTP RMF and launch modals cannot overlap, plus the fixed directional matrix in the update above. `Foreground`/`UIInteractionManager` retain lifecycle readiness; `PromoCoordinationService` keeps launch-source/unrelated-modal gates and owns the narrow cross-surface cooldown policy; `ModalPromptCoordinationManager` keeps provider-level onboarding eligibility, priority, existing modal cooldown, and presentation while holding the mutual-exclusion lease until exact-root release; NTP reports actual RMF visibility and waits unconsumed while blocked. PR 1 merged on 2026-08-04; final PR 2 contains lifecycle-safe scheduling, the NTP render gate, and all three host paths; PR 3 [#6217](https://github.com/duckduckgo/apple-browsers/pull/6217) contains the cooldown implementation/tests at `06a2417373` and intentionally contains no new Promo Queue telemetry. macOS `PromoService` is **not** extracted for iteration 1. Design: `promo-queue-docs/TECH_DESIGN_FINAL.md`.
...
May 26, 2026: scope debate on the main task ([iOS & Android] Implement Promo Queue, `1214299397742171`) — Ben S argued the queue is needed for **over-saturation control and visibility/instrumentation** ("which promos are shown, where, when"), not because of observed mobile collisions; RMFs were then out of scope because "there's no reliable way to predict when an RMF will appear." Jul 7–8: the initial direction was to reuse the macOS queue for iteration 1, extend it to the rest of the promos, and feed RMF into it. Jul 9: fresh thread on RMF gaps (see §2.3). Jul 10: kick-off. Subsequent code/history review changed the iteration-1 recommendation: preserve the existing iOS modal system and add bidirectional NTP/modal permits. The later August decision added only the fixed four-row cooldown matrix described above. The Jul 7–8 direction remains relevant historical context for iteration 3, not the implementation baseline.
...
- **iOS modal queue:** `iOS/DuckDuckGo/ModalPromptCoordination/` — seven providers in priority order: WinBack → Subscription (reinstaller) → Subscription (existing user) → AddressBarPicker → DefaultBrowser → What's New (consumes RMF `.modal`-surface content) → CPM opt-in; existing remotely tunable modal-to-modal cooldown; standard-launch gating in the service and provider-level onboarding gating in the manager. Iteration one additionally specifies fixed modal/RMF directional cooldowns in the service-owned coordination layer, not RMF.
