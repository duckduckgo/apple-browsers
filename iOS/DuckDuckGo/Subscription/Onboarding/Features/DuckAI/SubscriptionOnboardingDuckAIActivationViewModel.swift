...
@MainActor
final class SubscriptionOnboardingDuckAIActivationViewModel: ObservableObject {
    ...
    nonisolated init(modelProvider: SubscriptionOnboardingAIModelProviding = DefaultSubscriptionOnboardingAIModelProvider(),
    ...
    func onAppear() {
        // Fetch once; if the first attempt came back empty (offline etc.) a later appearance retries.
        guard models.isEmpty else { return }
        modelProvider.onModelsUpdated = { [weak self] in
            guard let self else { return }
            self.models = self.modelProvider.models
            // Adopt the store's resolved default, but never override a choice the customer has already made.
    ...
    /// Persists the committed model (default or tapped) so the launched web chat opens with it, then reports
