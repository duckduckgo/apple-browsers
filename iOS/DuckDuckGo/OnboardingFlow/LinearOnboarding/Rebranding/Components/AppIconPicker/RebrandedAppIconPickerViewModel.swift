...
final class RebrandedAppIconPickerViewModel: ObservableObject {
    ...
    private let appIconManager: RebrandedAppIconManaging

    init(appIconManager: RebrandedAppIconManaging = AppIconManager.shared) {
...
protocol RebrandedAppIconProviding {
    var appIcon: AppIcon { get }
}

protocol RebrandedAppIconManaging: RebrandedAppIconProviding {
    func changeAppIcon(_ appIcon: AppIcon, completionHandler: ((Error?) -> Void)?)
}

extension RebrandedAppIconManaging {
    func changeAppIcon(_ appIcon: AppIcon) {
        changeAppIcon(appIcon, completionHandler: nil)
    }
}

extension AppIconManager: RebrandedAppIconManaging {}
