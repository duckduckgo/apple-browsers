...
11. **Iteration-1 architecture changed after deeper integration review.** Iteration 1 no longer extracts macOS `PromoService` or creates `SharedPackages/PromoQueue`. The baseline is the existing iOS modal coordination plus a small app-level cross-surface permit coordinator and the fixed four-row cooldown policy above: `Foreground`/`UIInteractionManager` retain lifecycle readiness; the service retains launch-source/unrelated-modal gates and owns the narrow policy/store; the manager retains provider priority, existing modal cooldown, and presentation while holding the mutual-exclusion lease until exact-root release; NTP reports actual per-instance RMF visibility and is render-gated. Blocked RMF waits without shown/dismissed accounting, including until a 10-minute boundary after a modal. The modal is never retracted programmatically. `PromoService` remains an iteration-3 candidate/comparison, not shipped iOS infrastructure.
