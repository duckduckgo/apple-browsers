...
@Suite("EventHubPixelManager")
struct EventHubPixelManagerTests {
    ...
    func handleWebEventIncrementsMatchingCounter() {
        let f = EventHubManagerFixture.active(Self.dayConfig)
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: .new())
    ...
    func handleWebEventIgnoresUnknownEventType() {
        let f = EventHubManagerFixture.active(Self.dayConfig)
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("unrelated"), tabID: .new())
    ...
    func sameTabSameSourceIsDeduplicated() {
        ...
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: tab)
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: tab)
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: tab)
        #expect(f.count(of: Self.pixel1) == 1)
    }
    ...
    func closingATabClearsItsDedupState() {
        ...
        let tab = EventHubTabID.new()
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: tab)
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: tab) // deduped within the tab
    ...
    func timerFiresPixelWithBucketedCountAndAttributionPeriod() throws {
        ...
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: tab) // count = 2 -> bucket "1-2"
        ...
        #expect(pixel.parameters["attributionPeriod"] == EventHubManagerFixture.expectedAttribution(periodSeconds: 86400))
    }
    ...
    func skipsFiringWhenNoBucketMatches() {
        ...
        """
        let f = EventHubManagerFixture.active(config)
        f.advance(by: 60)
        #expect(f.fired.isEmpty)
    }
    ...
    func resetsStateAndStartsNewPeriodAfterFiring() throws {
        ...
        #expect(state.periodStartMillis == Int64(EventHubManagerFixture.start.addingTimeInterval(86400).timeIntervalSince1970 * 1000))
    }
    ...
    func attributionPeriodForHourlyPeriodIsStartOfHour() {
        ...
        """
        let f = EventHubManagerFixture.active(hourly)
        f.advance(by: 3600)
        #expect(f.fired.first?.parameters["attributionPeriod"] == EventHubManagerFixture.expectedAttribution(periodSeconds: 3600))
    }
    ...
    func runningPeriodUsesStoredConfigSourceNotLive() {
        ...
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("test"), tabID: .new())
        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("changed"), tabID: .new())
    ...
    func onConfigChangedDoesNotRegisterDisabledPixel() {
        ...
        """
        let f = EventHubManagerFixture.active(disabledPixel)
        #expect(f.manager.activePixelStates.isEmpty)
    }

    @Test("onConfigChanged with no settings registers no pixels")
    func onConfigChangedWithNoSettingsRegistersNoPixels() {
        let f = EventHubManagerFixture.active(Self.dayConfig, hasSettings: false)
        #expect(f.manager.activePixelStates.isEmpty)
    }
    ...
    func newPeriodIsNotStartedWhileBackgrounded() {
        let f = EventHubManagerFixture.background(Self.dayConfig)
    ...
    func removingAConfigTearsDownItsActivePeriodAndFiresNothing() throws {
        ...
        """
        let f = EventHubManagerFixture.active(json)

        f.manager.handleWebEvent(EventHubManagerFixture.webEvent("youtube_adBlocker"), tabID: .new())
