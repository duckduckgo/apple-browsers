    ...
    private func fetchShortcutsAsFavorites() -> [ImportedBookmarks.BookmarkOrFolder] {
        do {
            ...
            if preferences.isUsingAutoGeneratedShortcuts {
                ...
                let blocklistedShortcuts = preferences.autoGeneratedShortcutBlocklist
                let filteredShortcuts = autoGeneratedShortcuts.filter { shortcut in
                    guard let data = shortcut.url.data(using: .utf8) else {
                        return false
                    }
