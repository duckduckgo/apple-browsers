...
> **Current iteration-one clarification — 2026-08-12:** RMF still has no native caps/cooldowns. Q2 uses a startup-latched mode, one singular transient owner, and one service-owned logical RMF session with exactly one authorized renderer. Q3 implements the fixed iOS modal/NTP-RMF matrix in a service-owned policy using confirmed modal history plus one persisted confirmed-RMF timestamp, with checkpoint-only reconsideration. Same-message renderer handoff is one logical session; iOS 15/16 coordinated removal is intentionally synchronous and not animated. Promo Queue telemetry remains separate.
...
3. **No frequency capping/pacing in RMF**: no impression caps, native cross-message cooldown, or start/end dates (campaign windows = config edits). iOS modal and new directional iteration-one cooldowns live outside RMF in the client coordination layer. Iteration one uses a targeted app-level permit coordinator plus the four-row fixed policy above rather than macOS-style `ExternalPromo` integration. macOS's observation-only comparison remains at `macOS/DuckDuckGo/Promotions/Promos/PromoServiceFactory+RemoteMessage.swift` and `RemoteMessagePromoDelegate.swift:26-81`.
...
**A. ModalPromptCoordination (launch/foreground modal queue).** One modal max per evaluation, first-eligible-wins by priority, existing modal-to-modal global cooldown. The final iteration-one client layer also applies the fixed directional matrix above against NTP RMF.
...
- Modal-to-modal cooldown: default 24h, remote-tunable via privacy-config `iOSBrowserConfig.promptCooldownInterval` (`iOS/DuckDuckGo/ModalPromptCoordination/Cooldown/PromptCooldownIntervalProvider.swift:29-62`); last-shown key `com.duckduckgo.prompts.lastPromptShownTimestamp` (`.../Cooldown/PromptCooldownStore.swift:33-35`). Iteration one reuses this timestamp for modal→RMF and adds a separate last-confirmed-RMF timestamp for the fixed RMF-target rules.
...
2. **Modal coordination does not enforce a separate one-modal-per-session gate.** `didPresentModalPromptThisSession` is a courtesy signal consumed by the Duck.ai Sync promo; modal→modal repeat presentation is prevented by the remote-tunable global cooldown (default 24 hours). The later iteration-one decision adds the fixed modal/RMF matrix in a service-owned layer without changing that existing interval. `iOS/DuckDuckGo/ModalPromptCoordination/ModalPromptCoordinationManager.swift`; `iOS/DuckDuckGo/AppServices/PromoCoordinationService.swift`; `iOS/DuckDuckGo/ModalPromptCoordination/Cooldown/PromptCooldownIntervalProvider.swift`.
...
11. **Iteration-1 architecture changed after deeper integration review.** Iteration 1 no longer extracts macOS `PromoService` or creates `SharedPackages/PromoQueue`. The baseline is the existing iOS modal coordination plus one app-scoped owner and the fixed four-row cooldown policy: `Foreground`/`UIInteractionManager` retain lifecycle readiness; the service retains launch-source/unrelated-modal gates, owns one logical RMF session, authorizes one eligible renderer, and owns the narrow policy/store; the manager retains provider priority, existing modal cooldown, and exact-root presentation lifetime. NTP models report candidate/eligibility and render only a service-authorized presentation. Blocked RMF waits without shown/dismissed accounting. Same-message handoff retains one logical session through exact outgoing removal. The modal is never retracted programmatically. `PromoService` remains an iteration-3 candidate/comparison, not shipped iOS infrastructure.
