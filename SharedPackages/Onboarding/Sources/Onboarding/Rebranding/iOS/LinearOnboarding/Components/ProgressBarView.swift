...
#Preview("Onboarding Progress Indicator") {
    struct PreviewWrapper: View {
        @State var stepInfo = OnboardingRebranding.OnboardingProgressIndicator.StepInfo(currentStep: 1, totalSteps: 3)

        var body: some View {
            VStack(spacing: 100) {
                OnboardingRebranding.OnboardingProgressIndicator(stepInfo: stepInfo)

                Button(action: {
                    let nextStep = stepInfo.currentStep < stepInfo.totalSteps ? stepInfo.currentStep + 1 : 1
                    stepInfo = OnboardingRebranding.OnboardingProgressIndicator.StepInfo(currentStep: nextStep, totalSteps: stepInfo.totalSteps)
                }, label: {
...
#Preview("Progress Bar") {
    OnboardingRebranding.ProgressBarView(progress: 80)
