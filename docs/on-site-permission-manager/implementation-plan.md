...
5. **Recovery from OS-level denial** — a reminder dialog and System Settings deep links.
6. **Visual feedback** — a code-built grant animation and status iconography.
...
- No dialogs, no UI, no pixels, no animation.
...
## 6. Phase 3 — Camera/mic flow: dialogs, routing, recovery, animation, pixels
...
- The grant animation (code-built, no Lottie).
...
| Grant animation | lift the `CircleCheckView` pattern | see Step 5 |
...
mic flow. It is the deep-link precedent, and it belongs to another flow: **do not modify it and do
not regress it** (requirements §8, "no regressions to voice search").
...
**Case A — the user allows the site, then denies the OS prompt.** No animation. Decline the
...
There is a clean in-repo precedent: **`CircleCheckView`** in
`iOS/DuckDuckGo/OnboardingFlow/LinearOnboarding/Components/ComparisonTable/RebrandedOnboardingComparisonTableView.swift:253`.
It is code-built, spring-based, and has **no external consumers** (used only in its own file at
`:200` and its preview), so lifting the pattern is free. Take three things from it:

1. The animation constants — `:244-251`: `Animation.spring(duration: 0.45, bounce: 0.45)` for the
   scale, `.easeInOut(duration: 0.38)` for the draw, `0.2` delay between phases.
2. The sequenced `withAnimation` calls — `:322-337`.
3. **The reduce-motion escape hatch** — `@Environment(\.accessibilityReduceMotion)` at `:254`, and
   `snapToFinalState()` at `:317-321`, applied in both `.onAppear` (`:293`) and `.onChange` (`:305`).

The repo-wide transition idiom, with the same guard
(`iOS/DuckDuckGo/OnboardingFlow/LinearOnboarding/OnboardingView.swift:218-220`):
...
**The reduce-motion guard is not optional.** It is an accessibility basic, it is one line, and every
recent animation in this codebase has it.

Fires **after a grant only**. No animation on a site-level denial, and none on an OS-level denial.

Peer-review note worth keeping in mind: once granted, the iOS system status-bar indicators
(green/orange dots) are what users actually notice. This animation complements them; it is not
load-bearing. Do not over-invest.
...
| **Combined camera+microphone** | see below | see below | — | **3** |
...
**Combined camera+microphone (OQ-2 — no Figma design exists).** One dialog, because WebKit passes a
single decision handler for the pair. Build the title from the **bracketed dynamic-list pattern**
requirements §6 uses for the multi-permission footer — the bracket marks the part that grows:

> `DuckDuckGo needs to access your camera, [location, and microphone], if you want to use related
> features on this site.`

So for the combined dialog title, list both permissions in one sentence in the same
`your <a> and <b>` shape rather than inventing a second sentence. Icon: use the camera glyph
...
`Change Permissions`. Those belong to other projects. **Do not touch them and do not regress them**
(requirements §6).
...
- [ ] No dialog, no toast, and no animation can be reached with the flag off.
...
- **Nav title = the domain.** No in-page `Permissions for <domain>` header — see OQ-12 in §10.
