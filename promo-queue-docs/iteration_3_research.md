...
> **Iteration-one update — 2026-08-11:** RMF still has no native cooldown/frequency engine. The accepted Q2 endpoint is a startup-latched, singular-owner modal/NTP-RMF seam. Q3 will add the fixed modal→RMF 10m, RMF→RMF 10m, RMF→modal 24h, and existing modal→modal policy outside RMF using confirmed history and checkpoint-only reconsideration. The older Q3 implementation is superseded evidence, and telemetry remains a separate future project. This bounded iteration-one policy is not the long-term B/B+/C engine evaluated here.
