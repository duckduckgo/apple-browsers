...
| 11 | Voice Search denied-mic prompt | package dialog + app-side wiring at the two `NoMicPermissionAlert` call sites | restyle to the reminder design (Change Permissions / Hide Voice Search / Cancel), behind the flag — flag off keeps the legacy alert (in scope per DRI, 2026-08-28). Grant animation is **cut from v1** (same decision); OQ-6 is moot |
...
**Decision gates: none remain** (kick-off 2026-08-28 + DRI follow-up answers, same date). Implementation proceeds on documented sensible defaults throughout, to be finalized in a working build: combined-dialog copy defaults to `“<domain>” website wants to access your camera and microphone` with the standard three buttons; copy gaps use Figma verbatim plus the "needs to access your X" footer form, all new strings marked for a later copy pass; the per-site header uses the real domain (OQ-12); the OQ-17/18 membership/row defaults are adopted; the OQ-16 pixel set is **approved by the DRI (2026-08-28)** — final names in requirements §9; the OQ-7/OQ-21 privacy ping is assumed fine (send for the record when convenient). Camera-only/mic-only WebKit requests are assumed possible (`WKMediaCaptureType.camera`/`.microphone`). The **grant animation is cut from v1**; the **Voice Search denied-mic prompt restyle is added to PR 3**.
...
| **3. Camera/mic flow: dialogs, routing, recovery, Voice Search prompt, flow pixels** | 3-option dialog (cam/mic + combined-default copy) + FIFO presentation; `TabViewController` routing behind the flag (legacy matrix verbatim when off; Duck.ai branch preserved; AIChat untouched); Case A toast + Case B reminder dialog; in-use KVO; **Voice Search denied-mic prompt restyle** (reminder design, behind the flag); prompt/decision pixels. Grant animation cut from v1 | 2 | ~1.8k | 4.5 |
...
Any macOS changes; a shared cross-platform Permissions package; routing Duck.ai (either call site) through the model — explicit exception in both flag states; a geolocation subflag / staged camera-mic-first public release; permission types beyond camera/mic/location — named explicitly: notifications, pop-ups, autoplay, device motion, **DRM/protected media** (Android's fourth permission), and **screen/display capture**; address-bar indicator; **grant animation** (cut from v1, DRI 2026-08-28); per-session toggles / general mid-session mute (immediate revocation on deny/remove is in scope; live toggles are not); worker/service-worker Permissions API parity; Privacy Dashboard permission wiring; cross-device sync; porting macOS's private geolocation WebKit API.
