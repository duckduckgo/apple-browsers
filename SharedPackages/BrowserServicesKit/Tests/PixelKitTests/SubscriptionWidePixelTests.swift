...
final class SubscriptionWidePixelTests: XCTestCase {
    ...
    func testHighVolumeSubscriptionFlows() throws {
        let flowCount = 50
        let expectation = XCTestExpectation(description: "High volume flows")
        expectation.expectedFulfillmentCount = flowCount
        
        for i in 0..<flowCount {
            let subscriptionData = SubscriptionPurchaseWidePixelData(
                purchasePlatform: i % 2 == 0 ? .appstore : .stripe,
                subscriptionIdentifier: "volume-test-\(i)"
            )
            
            widePixel.startFlow(subscriptionData)
            
            DispatchQueue.global().async {
                self.widePixel.completeFlow(subscriptionData, finalStatus: .success) { success, error in
                    XCTAssertTrue(success)
                    expectation.fulfill()
                }
            }
        ...
        XCTAssertEqual(firedPixels.count, flowCount)
        
        // Verify platform distribution
        let appstoreCount = firedPixels.filter { $0.parameters["feature.data.ext.purchase_platform"] == "appstore" }.count
        let stripeCount = firedPixels.filter { $0.parameters["feature.data.ext.purchase_platform"] == "stripe" }.count
        
        XCTAssertEqual(appstoreCount + stripeCount, flowCount)
        XCTAssertTrue(abs(appstoreCount - stripeCount) <= 1) // Should be roughly equal
    }
    
    func testRealisticSubscriptionFlowSimulation() throws {
        let userCount = 20
        let expectation = XCTestExpectation(description: "Realistic subscription simulation")
        expectation.expectedFulfillmentCount = userCount
        
        let platforms: [SubscriptionPurchaseWidePixelData.PurchasePlatform] = [.appstore, .stripe]
        let outcomes: [WidePixelFinalStatus] = [.success, .failure, .cancelled]
        
        for i in 0..<userCount {
            DispatchQueue.global().async {
                do {
                    let platform = platforms[i % platforms.count]
                    var data = SubscriptionPurchaseWidePixelData(purchasePlatform: platform)
                    data.setContext(name: "simulation_user_\(i)")
                    
                    self.widePixel.startFlow(data)
                    
                    // Simulate account creation
                    self.widePixel.startMeasuring(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id, keyPath: \.createAccountDuration)
                    Thread.sleep(forTimeInterval: Double.random(in: 0.001...0.005))
                    self.widePixel.stopMeasuring(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id, keyPath: \.createAccountDuration)
                    
                    // Update with subscription info
                    data.subscriptionIdentifier = "sim-subscription-\(i)"
                    data.freeTrialEligible = i % 3 == 0
                    self.widePixel.updateFlow(data)
                    
                    // Simulate purchase
                    self.widePixel.startMeasuring(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id, keyPath: \.completePurchaseDuration)
                    Thread.sleep(forTimeInterval: Double.random(in: 0.001...0.01))
                    self.widePixel.stopMeasuring(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id, keyPath: \.completePurchaseDuration)
                    
                    // Simulate activation
                    self.widePixel.startMeasuring(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id, keyPath: \.activateAccountDuration)
                    Thread.sleep(forTimeInterval: Double.random(in: 0.001...0.003))
                    self.widePixel.stopMeasuring(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id, keyPath: \.activateAccountDuration)
                    
                    // Complete with outcome
                    let outcome = outcomes[i % outcomes.count]
                    let finalData = self.widePixel.getFlowData(SubscriptionPurchaseWidePixelData.self, contextID: data.contextData.id)!
                    
                    self.widePixel.completeFlow(finalData, finalStatus: outcome) { success, error in
                        XCTAssertTrue(success)
                        expectation.fulfill()
                    }
                } catch {
                    XCTFail("Realistic simulation failed: \(error)")
                }
            }
        }
        ...
        XCTAssertEqual(firedPixels.count, userCount)
        
        // Verify outcome distribution
        let successCount = firedPixels.filter { $0.parameters["feature.status"] == "SUCCESS" }.count
        let failureCount = firedPixels.filter { $0.parameters["feature.status"] == "FAILURE" }.count
        let cancelledCount = firedPixels.filter { $0.parameters["feature.status"] == "CANCELLED" }.count
        
        XCTAssertGreaterThan(successCount, 0)
        XCTAssertGreaterThan(failureCount, 0)  
        XCTAssertGreaterThan(cancelledCount, 0)
        XCTAssertEqual(successCount + failureCount + cancelledCount, userCount)
    }
    
    // MARK: - Subscription Flow Performance
    
    func testSubscriptionFlowPerformance() {
        measure {
            for i in 0..<20 {
                let data = SubscriptionPurchaseWidePixelData(
                    purchasePlatform: i % 2 == 0 ? .appstore : .stripe,
                    subscriptionIdentifier: "perf-test-\(i)"
                )
                self.widePixel.startFlow(data)
                ...
                // Complete flow
                self.widePixel.completeFlow(data, finalStatus: .success) { _, _ in }
            }
            widePixel.clearAllFlows()
        }
    }
