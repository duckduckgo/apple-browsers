...
final class SafariRedirectHandlerTests: XCTestCase {
    ...
    func testSubsequentRedirectsConvertUntilLoopThreshold() {
    ...
    func testThirdSubsequentRedirectRequestsStandardErrorFlow() {
        _ = handler.handleRedirect(to: xSafariHTTPSURL)
        _ = handler.handleRedirect(to: xSafariHTTPSURL) // count 1
        _ = handler.handleRedirect(to: xSafariHTTPSURL) // count 2
        _ = handler.handleRedirect(to: xSafariHTTPSURL) // count 3 -> loop

        XCTAssertEqual(delegate.loadedURLs.count, 3)
        XCTAssertEqual(delegate.loopErrorURLs.count, 1)
        XCTAssertEqual(delegate.loopErrorURLs.first?.scheme, "https")
        XCTAssertEqual(delegate.loopErrorURLs.first?.host, "example.com")
    }

    func testAdditionalRedirectsAfterLoopAreConsumedWithoutNewCallbacks() {
        _ = handler.handleRedirect(to: xSafariHTTPSURL)
        _ = handler.handleRedirect(to: xSafariHTTPSURL)
        _ = handler.handleRedirect(to: xSafariHTTPSURL)
        _ = handler.handleRedirect(to: xSafariHTTPSURL) // loop
