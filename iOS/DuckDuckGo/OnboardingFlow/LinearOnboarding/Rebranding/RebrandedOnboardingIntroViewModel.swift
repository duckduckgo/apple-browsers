//
//  RebrandedOnboardingIntroViewModel.swift
//  DuckDuckGo
//
...
@MainActor
final class RebrandedOnboardingIntroViewModel: ObservableObject {
    ...
    @Published private(set) var state: RebrandedOnboardingView.ViewState = .landing {
        didSet {
...
private extension RebrandedOnboardingIntroViewModel {
    ...
    func setViewState(introStep: OnboardingIntroStep) {
        func stepInfo() -> RebrandedOnboardingView.ViewState.Intro.StepInfo {
            ...
            return RebrandedOnboardingView.ViewState.Intro.StepInfo(currentStep: currentStepIndex, totalSteps: introSteps.count - 1)
        }

        let viewState = switch introStep {
        case .introDialog(let isReturningUser):
            RebrandedOnboardingView.ViewState.onboarding(.init(type: .startOnboardingDialog(canSkipTutorial: isReturningUser), step: .hidden))
        case .browserComparison:
            RebrandedOnboardingView.ViewState.onboarding(.init(type: .browsersComparisonDialog, step: stepInfo()))
        case .addToDockPromo:
            RebrandedOnboardingView.ViewState.onboarding(.init(type: .addToDockPromoDialog, step: stepInfo()))
        case .appIconSelection:
            RebrandedOnboardingView.ViewState.onboarding(.init(type: .chooseAppIconDialog, step: stepInfo()))
        case .addressBarPositionSelection:
            RebrandedOnboardingView.ViewState.onboarding(.init(type: .chooseAddressBarPositionDialog, step: stepInfo()))
        case .searchExperienceSelection:
            RebrandedOnboardingView.ViewState.onboarding(.init(type: .chooseSearchExperienceDialog, step: stepInfo()))
        }
