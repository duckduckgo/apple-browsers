...
| `sqlrc`, `sqlerc` | SQLite result codes carried by the error or any error in its chain. Read from `SQLiteResultCode` / `SQLiteExtendedResultCode` (`SecureStorageError`), falling back to `NSSQLiteErrorDomain` (Core Data) for `sqlrc`. Both come from the same error, outermost first, and never appear as a chain link |
