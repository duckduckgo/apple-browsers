...
> **Iteration-1 status note (affects several claims below).** The seven-provider modal chain and its modal-to-modal cooldown shipped to `main` **ungated** in Nov 2025 (`105dd6cf34` #2337), and PR 1 is on `main`. Q2 now uses a startup-latched singular owner and one service-owned logical RMF session that authorizes exactly one eligible renderer, retains ownership through an exact removal terminal, and removes the old per-model admission/mount graph. Q3 implements modal→RMF 10m, RMF→RMF 10m, RMF→modal 24h, and existing modal→modal policy from confirmed history with checkpoint-only reconsideration. Same-message renderer handoff confirms queue history once per logical session. Promo Queue telemetry remains separate. RMF itself still has no cooldown engine, so the framework findings below remain valid.
...
5. **Frequency capping is not missing — it exists 18 times over**: all 8 previously claimed implementations verified (2 of them the queues' own central mechanisms) plus 10 more found before the search was capped, i.e. **≥16 per-feature reimplementations outside the two central ones** (e.g. `SubscriptionPromoViewModel.swift:189` literally comments its 28-day window as a *"fallback when PromoQueue is off"*). The new iteration-one four-row client matrix is another deliberately narrow queue-layer rule, not an RMF capability. Caps/cooldown/priority remain queue-shaped gaps at portfolio scale; building them into RMF would duplicate the desktop queue's model (§2.2, §3 G3–G4).
...
8. **Measurement remains mostly render-time-only, per message.** RMF fires 13 pixel types on iOS, 6 on macOS; the desktop queue fires zero pixels of any kind. PR 3 intentionally adds no Promo Queue admission/cooldown telemetry after `1f12bf8a66`, so no system records collisions, eligibility delay, or eligible-but-not-shown delivery loss (§3 G11).
...
| Pacing / caps / cooldown | ✗ none natively. Persisted state is `shown` bool + `firstShownDate` + status only | same | The iOS iteration-one matrix is a separate app coordination policy, not RMF. Core Data model `CoreData/RemoteMessaging.xcdatamodeld/RemoteMessaging 3.xcdatamodel/contents:3-10`; zero grep hits for cooldown/frequency/impression/cap in the package |
...
| G3 | **No impression caps / re-show cooldowns / general global pacing in RMF** | The pacing every capped promo had to hand-roll | VPN menu dot cap 4 (`FreeTrialBadgePersistor.swift:30-34`), cookie opt-in cap 3 (`CookiePopupProtectionOptInModalPromptProvider.swift:96`), Duck.ai sync cap 5 (`SyncPromoManager.swift:61`) — plus 13 more (§4.3). Iteration one's fixed modal/NTP-RMF matrix is app-layer and narrower than this gap. | **Queue** — initiated cooldowns + history exist in the desktop queue; the iOS narrow matrix supplies only two timestamps/four directions | M in the queue layer; do **not** build in RMF |
...
| G11 | **No queue admission/delay/eligible-not-shown measurement** | The §7 measurement dispute cannot be settled with collision or delivery-loss data | RMF render-only pixels (§2.1); desktop queue zero pixels; PR 3 removes its two proposed aggregate collision events in `1f12bf8a66`, deferring all Promo Queue telemetry to a separate project | Both (new capability, queue-adjacent) | M + measurement review |
