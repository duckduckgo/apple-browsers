        ...
        for rule in matchingRules {
            ...
            for attribute in matchingRule.attributes {
                ...
                if result == .fail || result == .nextMessage {
                    Logger.remoteMessaging.error("First failing matching attribute for \(messageID, privacy: .public): \(String(describing: attribute), privacy: .public)")
                    break
                }
            }
    ...
    func evaluateExclusionRules(_ exclusionRules: [Int], messageID: String, fromRules rules: [RemoteConfigRule]) -> EvaluationResult {
        ...
        for rule in exclusionRules {
            ...
            if let percentile = matchingRule.targetPercentile, let messagePercentile = percentile.before {
                ...
                if userPercentile > messagePercentile {
                    Logger.remoteMessaging.error("Exclusion rule percentile check failed for message with ID \(messageID, privacy: .public)")
                    return .fail
                }
            }
            ...
            for attribute in matchingRule.attributes {
                result = evaluateAttribute(matchingAttribute: attribute)
                if result == .fail || result == .nextMessage {
                    Logger.remoteMessaging.error("First failing exclusion attribute for \(messageID, privacy: .public): \(String(describing: attribute), privacy: .public)")
                    break
                }
            }
