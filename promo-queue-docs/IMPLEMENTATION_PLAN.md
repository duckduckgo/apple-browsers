...
It does not create a general promo queue, change provider ordering, or extract the macOS implementation. The final PR 2 implementation does extend the provider adapter contract so prepared or retained prompts can be revalidated safely after a delay. An August 2026 product decision added a compiled, iteration-one-only directional cooldown policy between confirmed modal presentations and admitted NTP RMF appearances; PR 3 implements it in `06a2417373` rather than introducing a general RMF frequency engine.

**Current PR 3 status — 2026-08-06:** `bartosz/promo-q-3` is pushed at `06a2417373`. Commit `1f12bf8a66` removes the two previously proposed collision pixels because Promo Queue telemetry is deferred to a separate project. The final iteration-one app scope therefore includes the debug snapshot and existing RMF/provider accounting, but no new Promo Queue telemetry.

## Validation Verdict

The transactional-lease design remains the implementation direction for iteration 1. It matches the current main-actor modal flow and closes both known check-then-render races. Provider ordering, eligibility, modal-to-modal cooldown ownership, and RMF targeting/persistence remain unchanged; the service gains only the directional cross-surface cooldown policy specified below.
...
3. The arbiter is the only mutual-exclusion authority. It remains transient and owns no provider policy, RMF selection, cooldown, persistent history, or UIKit lifecycle. A separate service-owned policy/store owns directional cooldown checks and the new RMF timestamp.
4. A modal lease and one or more visible-promo leases cannot coexist. Multiple visible-promo leases may coexist for different surfaces only after later appearances pass the global RMF target cooldown; at most one lease may exist for a given `(surfaceID, promoType)`.
...
10. Feature-disabled behavior calls the legacy modal overload and preserves direct NTP mapping with its eager accounting callbacks. It bypasses all new cross-surface checks, RMF timestamp writes, provisional reservations, and timers while retaining confirmed timestamps for a later re-enable.
11. Cooldown denials retain eligible work and consume no provider or RMF shown/dismissed state. They are not lease conflicts. No new Promo Queue admission/cooldown telemetry is part of iteration one.

### Directional cooldown matrix and ownership

| Previous confirmed appearance | Next requested appearance | Required elapsed time |
| --- | --- | --- |
| Modal | NTP RMF | fixed 10 minutes |
| NTP RMF | NTP RMF | fixed 10 minutes, global across message IDs and NTP instances |
| NTP RMF | Modal | fixed 24 hours |
| Modal | Modal | existing `PromptCooldownManager` interval, remotely tunable and currently/default 24 hours |

The service-owned policy must reuse the existing persisted modal timestamp and add exactly one persisted last-confirmed-RMF timestamp. Store event times, not expiry dates. A modal event is written only by the existing confirmed-presentation accounting point (UIKit completion or exact-root adoption). An RMF event is written only on the first `onAppear` of the matching admitted render session. Selection, mapping, lease acquisition alone, denial, pending work, and failed/refused presentation never write either source event.

Use an injected clock. The exact boundary is eligible (`now >= timestamp + interval`). A wall clock that moves backward conservatively extends the cooldown, matching the existing modal behavior. The fixed 10-minute and RMF-to-modal 24-hour values are compiled constants; do not route them through RMF or privacy configuration. Only modal-to-modal retains the existing remote-tunable interval.

Before an RMF inner view is published, reserve the global RMF admission slot provisionally and identity-bind it to the candidate/render session. This prevents two NTP instances from passing the same cooldown check before either appears. The first matching appearance promotes the reservation to the persisted RMF timestamp; withdrawal first releases it without writing. Duplicate `onAppear`, physical remount, and same-ID refresh in one render session do not restart the cooldown. A later newly admitted render session, including the same message ID, is a new appearance. After 10 minutes another RMF may be admitted even if an earlier one remains visible; every active RMF lease still blocks modals until all leases release.

Return typed cooldown denial plus the next-eligible date. For RMF-target denials, retain the candidate and schedule/cancel one service-owned active-NTP retry at the earliest 10-minute boundary, while keeping existing lifecycle/render/configuration retries. Modal-target denials remain pending/reconsiderable but use foreground/checkpoint evaluation only after the 24-hour boundary; do not add a 24-hour timer.
...
Do not add priority, waiting order, restoration, or general policy to the arbiter. The two source-event timestamps and provisional RMF reservation belong to the separate cooldown component described above.
...
- identity-matched admission release and per-surface cooldown-retry cancellation;
- registration of a weak, synchronous retry callback per NTP surface.

Its typed admission result also carries cooldown denial with the earliest next-eligible date and a separate provisional-reservation denial. The service first acquires the transient visible lease, then checks the directional policy and obtains the global provisional RMF reservation before publication. It releases the lease immediately on policy denial, releases an unconfirmed reservation on withdrawal, and confirms only from the matching admitted `onAppear` callback.
...
3. reserve directional cooldown admission;
4. return a retained `PromoQueueVisiblePromoAdmission` wrapper owning both tokens on success.
...
1. If pending work exists, retry it before provider evaluation, subject to the same cross-surface cooldown checks.
2. Check the fixed RMF-to-modal 24-hour interval against the persisted RMF timestamp, then check the existing remote-tunable modal-to-modal cooldown against the existing modal timestamp.
...
11. Confirmed actual presentation, either UIKit completion or adoption of an already-attached prepared root:
    ...
    - records the existing modal timestamp exactly once; the cross-surface policy reuses this timestamp for modal-to-RMF checks.

`ModalPromptConfiguration` remains identity-stable and still carries no dismissal callback. The provider adapter contract adds prepared/retained revalidation and optional replacement so a delayed configuration is never presented solely because it was once eligible. Product eligibility, priority, modal-to-modal cooldown, and provider shown accounting remain with their existing owners; only the fixed directional cross-surface policy belongs to the service-owned cooldown component.
...
After modal dismissal/checkpoint release, a still-eligible RMF may remain blocked until the fixed 10-minute modal-to-RMF boundary. The service schedules one retry for that earliest boundary rather than accounting or consuming the candidate.
...
2. Run the manager and registered-NTP `willTransition(to: .disabled)` callbacks. The manager cancels/generation-invalidates committed work, clears pending coordination work, and preserves confirmed actual-presentation history; NTP models withdraw coordinated inner UI. Cancel the cooldown retry timer and clear provisional RMF reservations without writing history.
...
5. Publish the stable disabled state and lower the barrier. Retain both confirmed persisted timestamps, but bypass new cross-surface reads/writes while disabled.
...
4. Run the manager and NTP `didTransition(to: .enabled)` callbacks. The manager re-adopts a modal lease if its retained exact root is still attached; restore remaining cooldown state from confirmed timestamps; NTP models publish stable coordinated gates while public admission remains unavailable.
...
UI ready
  ...
  -> fixed RMF-to-modal cooldown
  -> existing modal-to-modal cooldown
  -> provider evaluation
```
...
No lease or pending-work restoration is required. Arbiter state, pending work, registrations, provisional RMF reservations, retry timers, and first-shown reservations are in-memory and disappear with the process. The existing last-modal and new last-RMF confirmed timestamps persist and are re-read after launch.
...
5. Acquire the visible lease for `VisiblePromoIdentity(surfaceID, .remoteMessage, messageID)`.
6. Check the fixed 10-minute modal/RMF-to-RMF target cooldown and atomically retain one global provisional RMF reservation; release the lease immediately on cooldown/provisional denial.
7. Only after reservation and lease acquisition, construct/publish the inner `HomeMessageViewModel`.
8. On the admitted inner view's first `onAppear`, record one shown appearance, promote the matching provisional reservation to the persisted RMF timestamp, and set `appearanceRecorded`.
9. When the outer gate leaves the visible render location, withdraw the inner view, release the exact matching lease, and release any unconfirmed provisional reservation without writing. Keep the eligible outer candidate so reappearance can retry.
...
- if the typed denial is cooldown-only, schedule one retry at the earliest 10-minute boundary;
- otherwise wait for a surface activation, foreground retry, RMF-change notification, or feature transition;
- emit no Promo Queue admission/cooldown telemetry.
...
1. confirm the matching provisional cooldown reservation and persist the RMF event timestamp once;
2. fire the normal shown pixel once;
3. read persisted `hasShown`;
4. if not persisted, atomically insert the message ID into the first-shown reservation set;
5. only the successful insertion fires unique shown and starts the RMF store update.
...
- two simultaneous NTP attempts cannot both pass the global provisional cooldown reservation; after 10 minutes a later admitted appearance of the same message produces another normal shown event but no new unique event;
...
| Week 5 hardening and rollout | Step 7 atomic accounting; Step 8 directional cooldowns; Step 9 behavioral tests; manual QA; Step 10 debug/docs; Step 11 privacy-config rollout and ship review |
...
3. `bartosz/promo-q-3` (**pushed and open as [#6217](https://github.com/duckduckgo/apple-browsers/pull/6217) from final PR 2; current tip `06a2417373`**): Step 7 atomic shown accounting, Step 8 directional cooldown implementation/tests (`06a2417373`), Step 9 behavioral integration coverage, one standalone stale alpha-animation completion guard, and Step 10 debug state. Commit `1f12bf8a66` removes the two collision pixels; telemetry is a separate future project. Temporary developer documentation stays only on the source branch.
...
The later privacy-configuration rollout in Step 11 is a separate pull request in
...
- consume the typed acquisition outcome without adding Promo Queue telemetry; only the visible-conflict result represents RMF blocking modal admission;
...
### Step 8 — Add directional modal/RMF cooldowns

Implementation status: complete on PR 3 in `06a2417373`. The production policy/store/scheduler and reservation token live together in `PromoQueueCooldownPolicy.swift`; the service owns them and exposes only the narrow admission result/wrapper to NTP code.

Files:

- `PromoCoordinationService.swift` and its narrow NTP coordination protocol;
- a small service-owned cooldown policy/store and injected clock/scheduler abstractions;
- the existing `PromptCooldownManager`/`PromptCooldownStore` protocol surface only as needed to reuse the modal timestamp without duplicating it;
- `NewTabPageMessagesModel.swift` and admitted-appearance callback plumbing;
- focused service, manager, NTP model, persistence, and integration tests;
- `project.pbxproj` for any new production/test files.

Work:

- implement the four-row matrix exactly, with fixed 10-minute RMF-target and fixed 24-hour RMF-to-modal constants;
- preserve the existing remotely tunable modal-to-modal interval;
- persist one last-confirmed-RMF event timestamp and reuse the existing modal event timestamp;
- add typed cooldown denial/next-eligible results without conflating them with lease denial;
- serialize concurrent RMF admission with one identity-bound global provisional reservation;
- confirm/persist only on the matching admitted render session's first appearance, and release without writing on withdrawal;
- schedule at most one earliest-boundary retry for active RMF candidates and no 24-hour modal timer;
- bypass new checks/writes/reservations/timers while disabled, clear transient cooldown state on disable, and resume confirmed history on re-enable;
- expose last-confirmed times, current next-eligible boundaries, provisional owner, and scheduled retry in the PR 3 debug snapshot without adding mutation controls;
- introduce no admission/cooldown telemetry; telemetry is outside iteration one.

Exit criteria:

- all four direction/boundary cases pass at just-before, exact, and just-after eligibility;
- persistence/relaunch and backward-clock behavior are deterministic;
- two concurrent NTPs cannot both pass before appearance, across IDs or for the same ID;
- duplicate appearance/remount/same-ID refresh does not extend cooldown, while a later admitted session does;
- a new RMF may appear after 10 minutes while an earlier RMF remains visible, but active leases continue blocking modals;
- denied/withdrawn/failed work consumes no provider, RMF, or timestamp state;
- feature-off remains byte-for-behavior legacy apart from the already-existing modal cooldown.

### Step 9 — Add behavioral integration coverage
...
### Step 10 — Add rollout debug state and docs

Implementation status: the debug snapshot is present on PR 3. The telemetry portion of the earlier plan was removed in `1f12bf8a66` and is no longer an iteration-one deliverable.
...
- existing arbiter/service tests for snapshot state;
...
- show the separate cooldown snapshot described in Step 8 alongside transient arbiter state;
- do not add history or lease mutation controls.

Promo Queue admission/cooldown telemetry is explicitly out of scope for iteration one. Commit `1f12bf8a66` removes both proposed collision events, their reporter, definitions, and focused tests. Existing RMF shown/dismiss/action accounting and provider-specific prompt/impression accounting remain unchanged. Any future queue telemetry requires a separate project, product/measurement contract, privacy review, implementation, and rollout plan.
...
- provider priority, eligibility, and modal-to-modal cooldown remain in the existing modal manager, while the service owns only the fixed directional cross-surface policy;
...
- no Promo Queue admission/cooldown pixel or reporter remains in the app change;
...
### Step 11 — Prepare the separate privacy-config rollout
...
| 7. Existing gates | Onboarding/provider eligibility, external launch/deep link, unrelated modal, OmniBar, provider order, and the existing modal-to-modal interval remain unchanged. |
| 8. Directional cooldown | All four rows enforce just-before/exact/just-after boundaries. Modal and RMF source events are written only on confirmed presentation/appearance. RMF admission is globally provisionally serialized; RMF-target denial schedules one earliest-boundary retry; modal-target denial remains checkpoint-driven. Denial consumes no provider/RMF accounting or timestamp state. |
| 9. Feature disabled/live toggle | Disabled uses the exact legacy path and bypasses new cooldown reads/writes/timers/reservations. Disable clears transient state but retains confirmed timestamps. Re-enable re-adopts an attached exact modal root and resumes remaining cooldown before active NTP retry. |
| 10. Background/termination | Background keeps active NTP and presentation-active modal leases; committed schedule is cancelled/pending without consuming state; stale callback cannot present. A new process starts with empty transient arbiter/reservation/timer state and restored confirmed timestamps. |
...
- two simultaneous NTP attempts cannot both pass the global provisional RMF reservation; after 10 minutes a later admitted appearance produces another normal shown and no additional unique shown;
...
- each matrix direction at one instant before, exactly at, and one instant after its boundary;
- modal confirmation and exact-root adoption write the existing modal timestamp once; failed/refused/pending/denied attempts do not;
- matching first RMF appearance persists once; withdrawal before appearance and duplicate/remount/same-ID callbacks do not write or extend;
- different IDs and different NTP instances share the same 10-minute RMF target cooldown;
- persistence survives relaunch and backward wall-clock movement conservatively extends the wait;
- after 10 minutes a second RMF may admit while the first remains visible, while the first lease continues to block modals;
- no new Promo Queue admission/cooldown telemetry is emitted; and
- one active-NTP cooldown timer is cancelled/replaced on candidate, activity, flag, or history changes; no modal 24-hour timer is created.
...
- new service-owned directional cooldown policy/store under `iOS/DuckDuckGo/ModalPromptCoordination/Cooldown/`, plus injected clock/retry scheduling as needed
- the existing `iOS/DuckDuckGo/ModalPromptCoordination/Cooldown/PromptCooldownStore.swift` protocol surface as needed to reuse, not duplicate, the modal timestamp
...
- each new cooldown policy/store production file in the app target and its focused tests in `iOS Unit Tests`;
...
4. **Modal root dismissed, NTP checkpoint occurs:** confirm an RMF remains pending/unaccounted until the 10-minute boundary, then appears once through the scheduled retry.
5. **Modal root dismissed, app foregrounded:** confirm RMF refresh/retry runs before new modal evaluation, subject to the 10-minute RMF target cooldown.
...
18. **Modal → RMF boundary:** test just before and exactly 10 minutes after confirmed modal presentation; only the exact-boundary attempt may render/account.
19. **RMF → RMF boundary:** open two NTP instances around the same time; confirm one provisional winner, then confirm another instance/message may appear at the exact 10-minute boundary even while the first remains visible.
20. **RMF → modal boundary:** remove all active RMF leases and confirm no modal provider is evaluated until an eligible foreground/checkpoint at least 24 hours after the latest confirmed RMF appearance.
21. **Persistence and flag:** relaunch and live-disable/re-enable during both cooldowns; confirm history remains, feature-off stays legacy, and re-enable resumes the remaining wait without a duplicate timer/write.

Inspect the extended debug screen during these scenarios and verify lease counts/phases, confirmed timestamps, next-eligible boundaries, provisional ownership, and the single scheduled retry.
...
2. modal provider/modal-to-modal cooldown/accounting preservation;
3. foreground/background and failed-presentation behavior;
4. all three NTP logical-visibility paths;
5. directional cooldown persistence, provisional admission, retry boundaries, RMF accounting, and live flag rollback;
6. debug state and rollout behavior.
...
2. land a separate privacy-configuration change for `promoQueue.features.iOSPromoPresentationCoordination`;
3. use the actual first containing iOS release as `minSupportedVersion`;
4. exercise local override/internal channels;
5. start with a small staged percentage;
6. inspect debug cooldown timestamps/next-eligible/reservation/timer state and the existing RMF shown/dismissed and provider-specific prompt/impression signals; iteration one adds no Promo Queue admission/cooldown telemetry;
7. increase only after the agreed observation window and ship review.
...
- the directional 10-minute/24-hour matrix is enforced from confirmed source events with persisted timestamps, one provisional RMF reservation, deterministic boundary retries, and no accounting on denial;
- provider order, onboarding policy, modal-to-modal cooldown interval, OmniBar routing, and provider accounting remain unchanged; prepared/retained prompt adapters revalidate before presentation;
- unit/integration coverage satisfies the behavioral matrix;
- the existing debug screen exposes current coordination state;
- no new Promo Queue telemetry is included; measurement is deferred to a separate project;
- privacy-config rollout and ship review are tracked as separate release actions;
- no general queue, generic RMF frequency engine, macOS extraction, or unrelated promo integration was added; persistence is limited to the existing modal timestamp plus one last-confirmed-RMF timestamp.
