...
final class AIChatSuggestionTests: XCTestCase {
    ...
    func testMatches_WithEmptyQuery_ReturnsTrue() {
        ...
        // When
        let result = suggestion.matches(query: "")

        // Then
        XCTAssertTrue(result)
    }

    func testMatches_WithMatchingQuery_ReturnsTrue() {
        // Given
        let suggestion = AIChatSuggestion(
            id: "1",
            title: "How to learn Swift programming",
            isPinned: false,
            chatId: "chat-1"
        )

        // When/Then
        XCTAssertTrue(suggestion.matches(query: "Swift"))
        XCTAssertTrue(suggestion.matches(query: "swift")) // case insensitive
        XCTAssertTrue(suggestion.matches(query: "SWIFT")) // case insensitive
        XCTAssertTrue(suggestion.matches(query: "learn"))
        XCTAssertTrue(suggestion.matches(query: "programming"))
    }
    ...
    func testMatches_WithNonMatchingQuery_ReturnsFalse() {
        ...
        // When/Then
        XCTAssertFalse(suggestion.matches(query: "Python"))
        XCTAssertFalse(suggestion.matches(query: "JavaScript"))
        XCTAssertFalse(suggestion.matches(query: "xyz123"))
    }

    func testMatches_WithPartialMatch_ReturnsTrue() {
        // Given
        let suggestion = AIChatSuggestion(
            id: "1",
        ...
        // When/Then
        XCTAssertTrue(suggestion.matches(query: "Swif"))
        XCTAssertTrue(suggestion.matches(query: "UI"))
        XCTAssertTrue(suggestion.matches(query: "basic"))
    }
