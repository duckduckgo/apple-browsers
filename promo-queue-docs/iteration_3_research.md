...
> **Iteration-one update — 2026-08-12:** RMF still has no native cooldown/frequency engine. Q2 is a startup-latched, singular-owner modal/NTP-RMF seam with one service-owned logical RMF session and one authorized renderer. Q3 implements the fixed modal→RMF 10m, RMF→RMF 10m, RMF→modal 24h, and existing modal→modal policy outside RMF using confirmed history and checkpoint-only reconsideration. Host eligibility and an exact removal terminal remain necessary; distributed per-model mount ownership is gone. Telemetry remains separate. This bounded policy is not the long-term B/B+/C engine evaluated here.
...
**Timeline note:** iteration 1 no longer extracts macOS `PromoService`. Its iOS baseline is deliberately narrow: `Foreground`/`UIInteractionManager` retain lifecycle readiness; the existing modal service/manager retain launch gates, provider priority, the existing modal cooldown, and presentation; the app-scoped service owns one logical RMF session, chooses one eligible renderer, and arbitrates it against the modal group; and its policy applies only the fixed matrix above. This is useful evidence for permit/policy layering, but **candidate B remains a future option**, not shipped/shared iOS infrastructure.
...
10. **iOS iteration 1 is not this queue:** it preserves `PromoCoordinationService`, `ModalPromptCoordinationManager`, all seven providers, and existing `PromptCooldownManager` behavior. A small app-level coordinator grants NTP/modal permits and applies only the fixed directional matrix above through a separate service-owned policy/store; once a modal commits it is never programmatically hidden, and RMF waits without shown/dismissed accounting until both lease and cooldown admission pass. PR 1 added disabled-by-default `FeatureFlag.promoPresentationCoordination`, now housed in the `FeatureFlags-iOS` package and mapped to `iOSPromoQueueSubfeature.iOSPromoPresentationCoordination`; remote enablement remains a separate rollout decision.
...
- the honest gap list Mark asked for: **what the desktop solution doesn't support today** — remote content, remote per-promo kill, cross-platform single implementation (Windows is parallel code), impression caps/expiry enforcement (history exists, policy doesn't), owner dashboards, FE/web surfaces on desktop NTP, and iOS/Android coverage. Iteration 1 supplies only a narrow iOS NTP/modal permit plus fixed cooldown seam, not PromoService coverage or a general policy engine.
...
- **Layering rule:** RMF = remote scheduling/content/targeting/kill for anything remote; queue = the only component that decides *visibility now* (arbitration, frequency, priority, context/severity), for RMF and non-RMF promos alike. Treat the iteration-one four-row policy as narrow precedent, not the completed long-term engine. Verify the seam works beyond the NTP card: how does a hypothetical RMF modal/badge enter the queue — one `ExternalPromo` per surface (macOS pattern) or per message?
