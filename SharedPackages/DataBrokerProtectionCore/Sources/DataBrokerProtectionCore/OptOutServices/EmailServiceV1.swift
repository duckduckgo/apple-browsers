...
public struct EmailServiceV1: EmailServiceV1Protocol {
    ...
    public func fetchEmailData(items: [EmailDataRequestItemV1]) async throws -> EmailDataResponseV1 {
        Logger.dataBrokerProtection.log("✉️ [EmailService] Fetching email data for \(items.count, privacy: .public) items")
        ...
        Logger.dataBrokerProtection.log("✉️ [EmailService] Email data API call successful for \(items.count, privacy: .public) items")

        do {
            return try JSONDecoder().decode(EmailDataResponseV1.self, from: data)
        } catch {
            Logger.dataBrokerProtection.error("✉️ [EmailService] Failed to decode email data response: \(error, privacy: .public)")
    ...
    public func deleteEmailData(items: [EmailDataRequestItemV1]) async throws {
        ...
        Logger.dataBrokerProtection.log("✉️ [EmailService] Deleting email data for \(items.count, privacy: .public) items")
