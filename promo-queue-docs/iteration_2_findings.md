...
> **Iteration-one status update — 2026-08-12:** The historical finding that RMF has no cooldown engine remains true. Q2 now uses a startup-latched mode, one singular transient owner, and one service-owned logical RMF session that authorizes one renderer. Q3 implements modal→RMF 10m, RMF→RMF 10m, RMF→modal 24h, and the existing modal→modal interval from confirmed history with checkpoint-only reconsideration. Host exposure remains a renderer-eligibility signal, while distributed per-model lease/mount tracking is gone. Promo Queue telemetry remains separate.
...
- **The appendix's “one modal/session” description is not a rule.** `didPresentModalPromptThisSession` is written and exposed as a courtesy signal, but it is never an eligibility gate. The remote-tunable global cooldown, default 24 hours, remains the modal→modal repeat-prevention mechanism. The later fixed modal/RMF matrix lives in a separate service-owned policy. `iOS/DuckDuckGo/ModalPromptCoordination/ModalPromptCoordinationManager.swift`; `iOS/DuckDuckGo/AppServices/PromoCoordinationService.swift`; `iOS/DuckDuckGo/ModalPromptCoordination/Cooldown/PromptCooldownIntervalProvider.swift`
...
|---|---|---|---|---|
| RMF NTP channel / win-back urgency | NTP on normal/after-idle fetch; local offer state feeds urgency. `iOS/DuckDuckGo/HomePageConfiguration.swift:56-82`; `SharedPackages/BrowserServicesKit/Sources/Subscription/WinBackOffer/WinBackOfferVisibilityManager.swift:96-109` | RMF JSON + hardcoded matcher vocabulary | RMF itself has no cap/global pace; iteration one layers fixed client modal/RMF cooldowns outside RMF. `SharedPackages/BrowserServicesKit/Sources/RemoteMessaging/RemoteMessagingStore.swift:255-275,324-369` | **(a)** already RMF; iteration-one NTP visibility/cooldown seam |
...
| Launch providers / iteration-one cross-surface pacing | Existing modal key `com.duckduckgo.prompts.lastPromptShownTimestamp`, reused for modal→RMF; one new last-confirmed-RMF timestamp supplies RMF→RMF/modal. Store source-event times only. `iOS/DuckDuckGo/ModalPromptCoordination/Cooldown/PromptCooldownStore.swift:31-60` |
...
**Corrected integration statement:** iteration 1 does not create an iOS `ExternalPromo` path or a shared `PromoQueue` package. It adds a narrow app-level permit coordinator plus a fixed directional cooldown policy between the existing launch-modal group and actually visible NTP RMF. Future NTP messages benefit at the surface level, but every new RMF/native surface still needs explicit visibility, accounting, arbitration, and any deliberately approved pacing integration. macOS's observation-only pattern is comparison evidence only: `macOS/DuckDuckGo/Promotions/Promos/PromoServiceFactory+RemoteMessage.swift:24-46`.
