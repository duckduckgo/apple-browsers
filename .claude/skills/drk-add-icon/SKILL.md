...
3. **Create the imageset/symbolset directory:**
   ...
   - Recolorable: `Recolorable/{size}px/{Title-Cased-filename-without-.svg}.symbolset/` — the directory is Title-Case (e.g. `Check-Recolorable-24.symbolset/`) even though the SVG filename inside stays lowercase. Match the existing repo convention.
4. **Copy the SVG into that directory** (filename unchanged).
...
- **Recolorable casing is asymmetric.** The SVG filename is lowercase (`check-recolorable-24.svg`), but the containing directory is **Title-Case** (`Check-Recolorable-24.symbolset/`) to match the convention used by Glyph and Color imageset directories. The `filename` reference inside `Contents.json` stays lowercase to match the actual SVG.
