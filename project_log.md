...
- Status: Final independent review is complete for the linear production stack `bartosz/promo-q-simp-2` (`27962e9e52`) → `bartosz/promo-q-simp-3` (`afa4ace811`) → `bartosz/promo-q-simp-4` (`0978e46ee9`). Verdict: **changes required**, proposal fidelity **9/10**. PR 2 still has one unguarded conditional legacy/iPad activation call that rebuilds legacy RMF state while `.promoPresentationCoordination` is disabled. The earlier claim that the legacy activation leak was fully fixed is superseded.
- Completed: Read-only branch/GitHub topology, tree-level documentation hygiene, exact layer sizes, architecture/feature-gate review, and independent XcodeBuildMCP builds/tests are recorded in `promo-queue-docs/final_code_review.md`. The self-contained executive appendix is `promo-queue-docs/final_feature_review.html`; it passed HTML5 structure, anchor/ARIA, offline-resource, responsive-containment, and print-rule audits. Direct in-app-browser rendering was unavailable because its security policy blocks local `file://` navigation.
- Next: Guard `MainViewController.onTextFieldWillBeginEditing` preparation to coordinated mode (preferably make concrete legacy preparation inert), add the host-level flag-off regression, review the fix across descendants, and rerun the affected source/SharedState plus integrated selections. Then execute the documented manual matrix before merge/rollout and fill the external PR/version/owner records.
- Blockers: Merge blocker — flag-off address-bar activation can perform a stateful legacy provider read and shared-source publication (`MainViewController.swift:5553`; `HomePageConfiguration.swift:108-111`). Manual feature-on/off scenarios, release-version confirmation, privacy-config review, cohort deployment, and rollout ownership remain human/external work.
