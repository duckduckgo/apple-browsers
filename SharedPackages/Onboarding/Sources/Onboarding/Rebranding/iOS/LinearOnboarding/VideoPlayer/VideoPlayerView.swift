...
struct PlayerView_Previews: PreviewProvider {
    ...
    struct PlayerPreview: View {
        static let videoURL = Bundle.module.url(forResource: "add-to-dock-demo", withExtension: "mp4", subdirectory: "Rebranding")!
        @State var model = OnboardingRebranding.VideoPlayerCoordinator(configuration: OnboardingRebranding.VideoPlayerConfiguration())

        var body: some View {
            OnboardingRebranding.PlayerView(
                coordinator: model
            )
            .onAppear(perform: {
