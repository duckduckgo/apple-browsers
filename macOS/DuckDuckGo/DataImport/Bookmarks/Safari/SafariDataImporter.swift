...
final class SafariDataImporter: DataImporter {
    ...
    private func importFavicons(from dataDirectoryURL: URL) async {
        ...
        case .success(let faviconsByURL):
            ...
            PixelKit.fire(GeneralPixel.dataImportSucceeded(action: .favicons, source: source, sourceVersion: sourceVersion), frequency: .dailyAndStandard)

        case .failure(let error):
            PixelKit.fire(GeneralPixel.dataImportFailed(source: source, sourceVersion: sourceVersion, error: error), frequency: .dailyAndStandard)
        }
    }
