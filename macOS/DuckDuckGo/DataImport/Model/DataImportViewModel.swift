    ...
    private mutating func mergeImportSummary(with summary: DataImportSummary) {
        ...
        for (dataType, result) in DataType.allCases.compactMap({ dataType in summary[dataType].map { (dataType, $0) } }) {
            ...
            case .success(let dataTypeSummary):
                ...
                PixelKit.fire(GeneralPixel.dataImportSucceeded(action: .init(dataType), source: importSource, sourceVersion: sourceVersion), frequency: .dailyAndStandard)
            case .failure(let error):
                ...
                PixelKit.fire(GeneralPixel.dataImportFailed(source: importSource, sourceVersion: sourceVersion, error: error), frequency: .dailyAndStandard)
            }
        }
        ...
        for error in summary.values {
            ...
            case let importError where (importError.underlyingError as? CocoaError)?.code == .fileReadNoPermission:
                ...
                if url != selectedProfile?.profileURL.appendingPathComponent(SafariDataImporter.bookmarksFileName) {
                    PixelKit.fire(GeneralPixel.dataImportFailed(source: importSource, sourceVersion: importSource.installedAppsMajorVersionDescription(selectedProfile: selectedProfile), error: importError), frequency: .dailyAndStandard)
                }
