...
> **Iteration-1 status note (affects several claims below).** The seven-provider modal chain and its cooldown shipped to `main` **ungated** in Nov 2025 (`105dd6cf34` #2337). PR 1 is on `main`: disabled-by-default `FeatureFlag.promoPresentationCoordination` maps to `iOSPromoQueueSubfeature.iOSPromoPresentationCoordination` in the `FeatureFlags-iOS` package; `PromoCoordinationService`, `PromoCoordinationFactory`, the app-scoped lease arbiter, modal admission/phases, NTP-facing admission/retry seam, and focused coverage landed in `7fdd4719a1`. PR 2 is final and awaiting review as [#6175](https://github.com/duckduckgo/apple-browsers/pull/6175) at `8d6d95438e`: it adds lifecycle-safe scheduling, prepared/retained-provider revalidation, the stable NTP render gate, all three host visibility/coverage/animation paths, weak lifecycle pruning, and exact feature-off regression coverage. `ADDING_PROMOS.md` describes the complete source-branch integration, including the remaining PR 3 accounting/observability work.
