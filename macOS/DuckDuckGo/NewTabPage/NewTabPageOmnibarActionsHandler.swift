...
final class NewTabPageOmnibarActionsHandler: NewTabPageOmnibarActionsHandling {
    ...
    func submitSearch(_ term: String, target: NewTabPage.NewTabPageDataModel.OpenTarget) {
        ...
        if isShiftPressed() {
            submitChat(term, target: isCommandPressed() ? .newTab : .sameTab, modelId: nil, images: nil, reasoningEffort: nil)
            return
        }
    ...
    func submitChat(_ chat: String, target: NewTabPage.NewTabPageDataModel.OpenTarget, modelId: String?, images: [NewTabPage.NewTabPageDataModel.SubmitChatImage]?, reasoningEffort: String?) {
        ...
        // Re-set prompt after tab opener to include images, model selection, and reasoning
        // effort (tab opener overwrites with a plain query)
        let nativeImages = images?.map { AIChatNativePrompt.NativePromptImage(data: $0.data, format: $0.format) }
        let nativePrompt = AIChatNativePrompt.queryPrompt(chat, autoSubmit: true, images: nativeImages, modelId: modelId, reasoningEffort: reasoningEffort)
